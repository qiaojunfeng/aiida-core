...
def get_explicit_kpoints_path(
    ...
    if dimension == 0:
        ...
        return [[0.0, 0.0, 0.0]], None, bravais_info
        ...
        for piece in value:
            if piece[0] in point_coordinates:
                ...
                point_coordinates[piece[0]] = change_reference(
                    reciprocal_cell, numpy.array([piece[1]]), to_cartesian=False
                )[0]
            else:
            ...
            if piece[2] in point_coordinates:
                ...
                point_coordinates[piece[2]] = change_reference(
                    reciprocal_cell, numpy.array([piece[3]]), to_cartesian=False
                )[0]
            else:
...
def find_bravais_info(cell, pbc, epsilon_length=_default_epsilon_length, epsilon_angle=_default_epsilon_angle):
    ...
    if dimension == 3:
        ...
        if comparison_length.count(True) == 3:
            ...
            bco1 = -(-(orci_the_a**2) + orci_the_b**2 + orci_the_c**2) / (4.0 * a**2)
            bco2 = -(orci_the_a**2 - orci_the_b**2 + orci_the_c**2) / (4.0 * a**2)
            bco3 = -(orci_the_a**2 + orci_the_b**2 - orci_the_c**2) / (4.0 * a**2)
            ...
            # body centered cubic #
            # =====================#
            elif comparison_angles.count(True) == 3 and a_are_equals(cosa, -1.0 / 3.0):
                bravais_info = {
                    'short_name': 'bcc',
            ...
            # ==========================#
                ...
                the_c = numpy.sqrt(-4.0 * ref_ang * (a**2))
                the_a = numpy.sqrt(2.0 * a**2 - (the_c**2) / 2.0)

                if the_c < the_a:
                    bravais_info['variation'] = 'bct1'
                    bravais_info['extra'] = {'eta': (1.0 + (the_c / the_a) ** 2) / 4.0}
                else:
                    bravais_info['variation'] = 'bct2'
                    bravais_info['extra'] = {
                        'eta': (1.0 + (the_a / the_c) ** 2) / 4.0,
                        'csi': ((the_a / the_c) ** 2) / 2.0,
                    }
            ...
            # ============================#
                ...
                any([a_are_equals(cosa, bco1), a_are_equals(cosb, bco1), a_are_equals(cosc, bco1)])
                and any([a_are_equals(cosa, bco2), a_are_equals(cosb, bco2), a_are_equals(cosc, bco2)])
                and any([a_are_equals(cosa, bco3), a_are_equals(cosb, bco3), a_are_equals(cosc, bco3)])
            ):
                bravais_info = {
                    'short_name': 'orci',
                ...
                bravais_info['extra'] = {
                    'csi': (1.0 + (orci_the_a / orci_the_c) ** 2) / 4.0,
                    'eta': (1.0 + (orci_the_b / orci_the_c) ** 2) / 4.0,
                    'dlt': (orci_the_b**2 - orci_the_a**2) / (4.0 * orci_the_c**2),
                    'mu': (orci_the_a**2 + orci_the_b**2) / (4.0 * orci_the_c**2),
                }
            ...
            elif comparison_angles.count(True) == 1 and any(
                [a_are_equals(cosa, _90), a_are_equals(cosb, _90), a_are_equals(cosc, _90)]
            ):
            ...
            # =======================#
                ...
                the_b = numpy.linalg.norm(the_a1 + the_a2)
                the_a = numpy.linalg.norm(the_a1 - the_a2)
                the_cosa = 2.0 * numpy.linalg.norm(the_a1) / the_b * the_ka

                if a_are_equals(the_ka, _90):  # order matters: has to be before the check on mclc1
                    bravais_info['variation'] = 'mclc2'
                    csi = (2.0 - the_b * the_cosa / the_c) / (4.0 * (1.0 - the_cosa**2))
                    psi = 0.75 - the_a**2 / (4.0 * the_b * (1.0 - the_cosa**2))
                    bravais_info['extra'] = {
                        'csi': csi,
                        'eta': 0.5 + 2.0 * csi * the_c * the_cosa / the_b,
                        'psi': psi,
                    ...
                    x = the_b * the_cosa / the_c + the_b**2 * (1.0 - the_cosa**2) / the_a**2
                    if a_are_equals(x, 1.0):
                        bravais_info['variation'] = 'mclc4'  # order matters here too
                        mu = (1.0 + (the_b / the_a) ** 2) / 4.0
                        dlt = the_b * the_c * the_cosa / (2.0 * the_a**2)
                        csi = mu - 0.25 + (1.0 - the_b * the_cosa / the_c) / (4.0 * (1.0 - the_cosa**2))
                        eta = 0.5 + 2.0 * csi * the_c * the_cosa / the_b
                        phi = 1.0 + eta - 2.0 * mu
                        psi = eta - 2.0 * dlt
                        bravais_info['extra'] = {
                        ...
                        csi = ((the_b / the_a) ** 2 + (1.0 - the_b * the_cosa / the_c) / (1.0 - the_cosa**2)) / 4.0
                        eta = 0.5 + 2.0 * csi * the_c * the_cosa / the_b
                        mu = eta / 2.0 + the_b**2 / 4.0 / the_a**2 - the_b * the_c * the_cosa / 2.0 / the_a**2
                        nu = 2.0 * mu - csi
                        omg = (
                            (4.0 * nu - 1.0 - the_b**2 * (1.0 - the_cosa**2) / the_a**2)
                            * the_c
                            / (2.0 * the_b * the_cosa)
                        )
                        dlt = csi * the_c * the_cosa / the_b + omg / 2.0 - 0.25
                        rho = 1.0 - csi * the_a**2 / the_b**2
                        bravais_info['extra'] = {
                            'mu': mu,
                            'dlt': dlt,
                            'csi': csi,
            ...
            # ============#
                ...
                if not a_are_equals(cosa, _90):
                    ...
                    if lens.index(the_a) == 1:
                        ...
                        bravais_info['permutation'] = [2, 1, 0]
                eta = (1.0 - the_b * the_cosa / the_c) / (2.0 * (1.0 - the_cosa**2))
                bravais_info['extra'] = {
                    'eta': eta,
            ...
            # ============================#
                ...
                any([a_are_equals(cosa, fco1), a_are_equals(cosb, fco1), a_are_equals(cosc, fco1)])
                and any([a_are_equals(cosa, fco2), a_are_equals(cosb, fco2), a_are_equals(cosc, fco2)])
                and any([a_are_equals(cosa, fco3), a_are_equals(cosb, fco3), a_are_equals(cosc, fco3)])
            ):
                bravais_info = {
                    'short_name': 'orcf',
                ...
                fco4 = 1.0 / the_a**2 - 1.0 / the_b**2 - 1.0 / the_c**2
                # orcf3
                if a_are_equals(fco4, 0.0):
                    bravais_info['variation'] = 'orcf3'  # order matters
                    bravais_info['extra'] = {
                        'csi': (1.0 + (the_a / the_b) ** 2 - (the_a / the_c) ** 2) / 4.0,
                        'eta': (1.0 + (the_a / the_b) ** 2 + (the_a / the_c) ** 2) / 4.0,
                    }
                # orcf1
                elif fco4 > 0.0:
                ...
                # orcf2
                    ...
                    bravais_info['extra'] = {
                        'eta': (1.0 + (the_a / the_b) ** 2 - (the_a / the_c) ** 2) / 4.0,
                        'dlt': (1.0 + (the_b / the_a) ** 2 + (the_b / the_c) ** 2) / 4.0,
                        'phi': (1.0 + (the_c / the_b) ** 2 - (the_c / the_a) ** 2) / 4.0,
                    }
        ...
        if bravais_info['short_name'] == 'tri':
            ...
            if the_cosa < 0.0 and the_cosb < 0.0:
                if a_are_equals(the_cosc, 0.0):
            ...
            elif the_cosa > 0.0 and the_cosb > 0.0:
                if a_are_equals(the_cosc, 0.0):
        ...
        # ======================#
        elif comparison_length and l_are_equals(numpy.dot(vectors[0] + vectors[1], vectors[0] - vectors[1]), 0.0):
            bravais_info = {
                'short_name': 'recc',
...
def get_kpoints_path(
    ...
    if dimension == 3:
        ...
        # face centered cubic
            ...
            special_points = {
                'G': [0.0, 0.0, 0.0],
                'K': [3.0 / 8.0, 3.0 / 8.0, 0.75],
                'L': [0.5, 0.5, 0.5],
                'U': [5.0 / 8.0, 0.25, 5.0 / 8.0],
                'W': [0.5, 0.25, 0.75],
                'X': [0.5, 0.0, 0.5],
            }
            path = [
        ...
        # monoclinic
            ...
            if bravais_info['variation'] == 'mclc1':
                ...
                special_points = {
                    ...
                    'N': [0.5, 0.0, 0.0],
                    'N1': [0.0, -0.5, 0.0],
                    'F': [1.0 - csi, 1.0 - csi, 1.0 - eta],
                    'F1': [csi, csi, eta],
                    'F2': [csi, -csi, 1.0 - eta],
                    'F3': [1.0 - csi, -csi, 1.0 - eta],
                    'I': [phi, 1.0 - phi, 0.5],
                ...
                special_points = {
                    'G': [0.0, 0.0, 0.0],
                    'F': [1.0 - phi, 1 - phi, 1.0 - psi],
                    'F1': [phi, phi - 1.0, psi],
                    'F2': [1.0 - phi, -phi, 1.0 - psi],
                    'H': [csi, csi, eta],
                    'H1': [1.0 - csi, -csi, 1.0 - eta],
                    'H2': [-csi, -csi, 1.0 - eta],
                    'I': [0.5, -0.5, 0.5],
                    'M': [0.5, 0.0, 0.5],
                    'N': [0.5, 0.0, 0.0],
        ...
        if bravais_info['index'] == 1:
            ...
            special_points = {
                ...
                'X': [0.5, 0.5, 0.0],
                'Y1': [0.25, 0.75, 0.0],
                'Y': [-0.25, 0.25, 0.0],  # typo in p. 404 of Ramirez & Bohm (should be Y=(0,1/4))
                'C': [0.0, 0.5, 0.0],
            }
            path = [
                ('Y1', 'X'),
